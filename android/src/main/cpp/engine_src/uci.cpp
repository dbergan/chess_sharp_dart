...
void UCI::loop(int argc, char* argv[]) {
  ...
  do {
      ...
      // If we received command line arguments, treat them as the command
      // In the original loop logic, cmd is built from argv before the loop.
      // If argc > 1, cmd is already populated.
      
      execute_command(pos, states, cmd, banmoves);

      // Check for quit token to exit loop
      std::istringstream is(cmd);
      token.clear();
      is >> skipws >> token;
      
      // Update argc if necessary (logic from original loop regarding "uci" command resetting argc)
      if (token == "uci" || token == "usi" || token == "ucci" || token == "xboard" || token == "ucicyclone" || token == "load") {
          argc = 1;
      }
...
void UCI::execute_command(Position& pos, StateListPtr& states, std::string cmd, std::vector<Move>& banmoves) {
    std::string token;
    std::istringstream is(cmd);
