...
namespace {
  ...
  ExtMove* generate_all(const Position& pos, ExtMove* moveList) {
    ...
    if (pos.can_move_into_check() || Type != EVASIONS || !more_than_one(pos.checkers() & ~pos.non_sliding_riders()))
    {
    ...
    // King moves
    if (pos.count<KING>(Us) && (pos.can_move_into_check() || (!Checks || pos.blockers_for_king(~Us) & ksq)))
    {
