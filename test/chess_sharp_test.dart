...
void main() {
  ...
  test('Chess♯ - test pgn & fen import/export', () {
    ...
    testPGN(
      './data/2022-03-24 DB v John.pgn',
      finalFen: '8/8/p7/8/8/8/2k5/8 w - - 0 71',
    );

    testPGN(
      './data/2022-09-03 Caleb v DB.pgn',
      finalFen: 'kbb3r1/ppp4r/4pp2/8/P1N1PB2/1BP1R1pq/1P3P1P/3Q3K w - - 0 31',
    );

    testPGN(
      './data/2023-03-06 Jim v DB.pgn',
      finalFen: '8/3k4/bp2p3/4p3/PBRrP3/1P6/7P/5K2 w - - 6 53',
    );

    testPGN(
      './data/2023-03-09 Raphael v DB.pgn',
      finalFen: 'r3brq1/1kp1Rp2/2bQ4/p7/p7/2PP1PP1/PP1P3P/K3RN2 b - - 0 26',
    );

    testPGN(
      './data/2023-04-12 Jim v DB.pgn',
      finalFen: '3r3k/6p1/p1q3bp/8/5p2/2P2P2/PP1KQ3/7R w - - 1 42',
    );

    testPGN(
      './data/2023-04-17 DB v Jim.pgn',
      finalFen: '7k/p4r1p/bp3Np1/4R1P1/1B5Q/1P5P/KBq5/8 b - - 2 43',
    );

    testPGN(
      './data/2023-04-20 DB v Raphael.pgn',
      finalFen: '8/3R2pp/p3n3/P1n5/1kp5/8/2K3PP/8 w - - 8 48',
    );

    testPGN(
      './data/2023-04-21 DB v Hudson.pgn',
      finalFen: '7k/1R5P/P7/8/2p5/2P5/1P3P2/K7 b - - 0 49',
    );

    testPGN(
      './data/2023-04-24 Jim v DB.pgn',
      finalFen: '1k2Q1b1/1pr4p/8/8/4pP2/4P1P1/P6P/KN5q b - - 9 47',
    );

    testPGN(
      './data/2023-05-01 Jim v DB.pgn',
      finalFen:
          'k4b2/p1p3pp/b1p2p2/1r1pp3/8/4P1P1/1PPP1P1P/KRBN1QR1[rq] w - - 0 18',
    );

    testPGN(
      './data/2023-05-01 Jim v DB2.pgn',
      finalFen: '1r3B1k/2p2b2/3p1Q2/p1P1P3/3P2P1/P1n5/1rp2P2/K1R5 b - - 1 46',
    );

    testPGN(
      './data/2023-08-15 Jack v Owen Chaplain.pgn',
      finalFen: '3k4/p7/4p2r/2p3p1/8/4bBP1/P6P/1Q6 w - - 0 44',
    );

    testPGN(
      './data/2023-08-17 Jack v Ethan.pgn',
      finalFen: '6rr/ppppk1bN/4ppQ1/8/2B2P2/1BP1P3/PP1P4/2K4R[bq] b - - 13 32',
    );

    testPGN(
      './data/2023-05-13 John Bergan v Joseph Wynstra.pgn',
      finalFen: 'r2r4/p1bkn1Q1/3n4/3N4/4p3/4PP2/PP1P2PP/2BR2KR b - - 0 32',
    );

    testPGN(
      './data/2023-05-13 William Ristau v John Bergan.pgn',
      finalFen: '6b1/2kp3p/1pp3p1/5N2/1P2B3/1N5P/PK2R1P1/8 b - - 0 48',
    );

    testPGN(
      './data/2023-05-13 John Bergan v Carlos Casanova.pgn',
      finalFen: '1k3n2/p1pp4/1p1np3/4Pp2/B7/2P1P2p/PP1PQ1rP/1R2B3 w - - 0 31',
    );

    testPGN(
      './data/2023-05-13 David Bergan v Jim Wynstra.pgn',
      finalFen: '1kr5/pp6/3p4/1np5/4PB2/2q5/P1N3PQ/1K3R1R b - - 3 35',
    );

    testPGN(
      './data/2023-05-13 David Bergan v Raphael Neff.pgn',
      finalFen: '6k1/pp1p1ppp/2p5/1q3P2/5b2/n1P5/PPP5/3BQ2R w - - 0 30',
    );

    testPGN(
      './data/2023-05-13 Michael Bergan v David Bergan.pgn',
      finalFen: '8/5R2/1p2R3/1k2P3/2PP4/3K1P2/r7/8 b - - 0 50',
    );

    testPGN(
      './data/2023-05-13 Raphael Neff v Jim Wynstra.pgn',
      finalFen: 'r3q1k1/1Q3pbp/p4bp1/1p6/1NnP4/1RB1P1P1/P3NP1P/6K1 b - - 5 35',
    );

    testPGN(
      './data/2023-05-13 Jim Wynstra v Michael Bergan.pgn',
      finalFen: 'n1Q3bq/p5p1/kp1B1p1p/4pP2/3pP3/3P1N2/2P1N1PP/3K4 b - - 0 33',
    );

    testPGN(
      './data/2023-05-13 Raphael Neff v Michael Bergan.pgn',
      finalFen:
          '1kb1B1q1/p1p2p1p/1n2p1p1/1NQn4/3p1P2/3P4/PPP1P3/K2R4 b - - 0 28',
    );

    testPGN(
      './data/2023-06-12 Jim Wynstra v David Bergan.pgn',
      finalFen: 'q4bk1/5ppp/p7/1p2n3/3Qb3/8/PPP2PPP/1K3B2 w - - 2 28',
    );

    testPGN(
      './data/2023-10-02 David Bergan v Jim Wynstra.pgn',
      finalFen: '8/pp2Bp1r/2p3p1/3pN3/3P4/2P5/PP5K/8 b - - 0 41',
    );

    testPGN(
      './data/2023-10-21 David Bergan v Elliott Neff.pgn',
      finalFen: '8/8/8/3kn1pB/4p2p/4P2b/8/3K2B1 w - - 14 77',
    );

    testPGN(
      './data/2023-12-13 David Bergan v Jerry Bergan.pgn',
      finalFen: '6k1/6P1/6K1/8/8/8/8/8 b - - 2 83',
    );

    testPGN(
      './data/2024-01-20 Jim Wynstra v David Bergan.pgn',
      finalFen: '8/8/1n6/pk6/8/1K6/1N6/8 w - - 14 83',
    );

    testPGN(
      './data/2024-03-03 Chess Double Flat.pgn',
      finalFen: 'Q7/p1ppppp1/1p5p/8/8/6P1/PPPPPP1P/K7[q] b - - 0 4',
    );

    testPGN(
      './data/2024-05-13 Pawn takes King and promotes.pgn',
      finalFen: '3k4/pppp1ppp/8/8/8/8/PPP1PPPP/R1BBq~RNN[Qnnbbrrq] w - - 0 8',
    );
  });
  test('Chess♯ - test randomMove and randomUci', () {
    ...
    final Map<String, int> parsedMoveCounts = {};
    final Map<String, int> captureMoveCounts = {};
    final Map<String, int> dontCaptureKingMoveCounts = {};
    final Map<String, int> captureButNotKingMoveCounts = {};
    const trials = 10;

    for (int i = 0; i < trials; i++) {
      final move = a.randomMove();
      expect(move, isA<Move>());
      expect(a.isLegal(move), true);
      moveCounts[move.uci] = (moveCounts[move.uci] ?? 0) + 1;

      final uci = a.randomUci();
      expect(uci, isA<String>());

      final parsedMove = Move.parse(uci);
      expect(parsedMove, isNotNull);
      expect(a.isLegal(parsedMove!), true);
      parsedMoveCounts[parsedMove.uci] =
          (parsedMoveCounts[parsedMove.uci] ?? 0) + 1;

      // Test with flags
      final captureMove = a.randomMove(preferCapture: true);
      expect(a.isLegal(captureMove), true);
      captureMoveCounts[captureMove.uci] =
          (captureMoveCounts[captureMove.uci] ?? 0) + 1;

      final dontCaptureKingMove = a.randomMove(dontCaptureKing: true);
      expect(a.isLegal(dontCaptureKingMove), true);
      if (a.board.kingOf(a.turn.opposite) != null) {
        expect(dontCaptureKingMove.to, isNot(a.board.kingOf(a.turn.opposite)));
      }
      dontCaptureKingMoveCounts[dontCaptureKingMove.uci] =
          (dontCaptureKingMoveCounts[dontCaptureKingMove.uci] ?? 0) + 1;

      final captureButNotKingMove = a.randomMove(
        preferCapture: true,
        dontCaptureKing: true,
      );
      expect(a.isLegal(captureButNotKingMove), true);
      if (a.board.kingOf(a.turn.opposite) != null) {
        expect(
          captureButNotKingMove.to,
          isNot(a.board.kingOf(a.turn.opposite)),
        );
      }
      captureButNotKingMoveCounts[captureButNotKingMove.uci] =
          (captureButNotKingMoveCounts[captureButNotKingMove.uci] ?? 0) + 1;

      if (i % 5 == 0) print('$i out of $trials');
    }

    void printSummary(String label, Map<String, int> counts) {
      print('\n$label:');
      final sortedEntries = counts.entries.toList()
        ..sort((a, b) => b.value.compareTo(a.value));
      for (final entry in sortedEntries) {
        print('  ${entry.key.padRight(6)}: ${entry.value}');
      }
    }

    print('\nMove selection summary (out of $trials trials):');
    printSummary('randomMove()', moveCounts);
    printSummary('randomUci() -> Move.parse()', parsedMoveCounts);
    printSummary('randomMove(preferCapture: true)', captureMoveCounts);
    printSummary(
      'randomMove(dontCaptureKing: true)',
      dontCaptureKingMoveCounts,
    );
    printSummary(
      'randomMove(preferCapture: true, dontCaptureKing: true)',
      captureButNotKingMoveCounts,
    );
  });
  ...
  test('Chess♯ - test cpuMove', () async {
    ...
    for (int level = 0; level <= 20; level++) {
      final (move, evaluation) = await a.cpuMoveAndEval(
      ...
      print('Level $level, $milliseconds ms: ${move.uci} (eval: $evaluation)');
    }
