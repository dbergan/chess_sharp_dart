...
void main() {
  group('Position', () {
    test('implements hashCode/==', () {
      ...
      expect(
        Chess.initial,
        isNot(Chess.initial.play(NormalMove.fromUci('e2e4'))),
      );
    });

    test('Chess.toString()', () {
      expect(
        Chess.initial.toString(),
  ...
  group('san', () {
    ...
    test('makeSan with scholar mate', () {
      ...
      final (_, sans) = moves.fold<(Position, List<String>)>(
        (Chess.initial, []),
        (acc, e) {
          final (pos, sans) = acc;
    ...
    test('opening knight moves', () {
      const legalSans = ['Na3', 'Nc3', 'Nf3', 'Nh3'];

      const illegalSans = ['Ba3', 'Bc3', 'Bf3', 'Bh3', 'Ne4', 'Nb1', 'Ng1'];
  ...
  group('Chess', () {
    ...
    test('isInsufficientMaterial', () {
      expect(
        Chess.fromSetup(
          Setup.parseFen('5K2/8/8/1B6/8/k7/6b1/8 w - - 0 39'),
      ...
      expect(
        Chess.fromSetup(
          Setup.parseFen('3b4/8/8/6b1/8/8/R7/K1k5 w - - 0 1'),
        ).isInsufficientMaterial,
        false,
      );
    });
    ...
    test('most known legal moves', () {
      expect(
        ...
        ).legalMoves.values.fold<int>(0, (value, el) => value + el.size),
        218,
      );
    });
    ...
    test('castling legal moves', () {
      ...
      expect(
        pos.legalMovesOf(Square.e1),
        SquareSet.fromBigInt(BigInt.parse('00000000000000A9', radix: 16)),
      );
    });
...
'''),
    ...
    test('isStalemate', () {
      ...
      expect(
        Chess.fromSetup(
          Setup.parseFen('8/8/8/8/8/1pk5/p7/K7 w - - 0 70'),
        ).isStalemate,
        true,
      );
    });
    ...
    test('isLegal', () {
      expect(
        Chess.initial.isLegal(const NormalMove(from: Square.e2, to: Square.e4)),
        true,
      );
      expect(
        Chess.initial.isLegal(const NormalMove(from: Square.e4, to: Square.f4)),
        false,
      );
    ...
    test('isLegal alternate castling moves', () {
      ...
      expect(
        pos.isLegal(const NormalMove(from: Square.e1, to: Square.g1)),
        true,
      );
      expect(
        pos.isLegal(const NormalMove(from: Square.e1, to: Square.c1)),
        true,
      );
      expect(
        pos.isLegal(const NormalMove(from: Square.e1, to: Square.h1)),
        true,
      );
      expect(
        pos.isLegal(const NormalMove(from: Square.e1, to: Square.a1)),
        true,
      );
      // illegal move
      expect(
        pos.isLegal(const NormalMove(from: Square.e1, to: Square.a2)),
        false,
      );

      final blackPos = Chess.fromSetup(
        Setup.parseFen(
          'r3k2r/pppbbppp/2n2n2/3pp2Q/3PP2q/2N2N2/PPPBBPPP/R4RK1 b kq - 11 8',
        ),
      );
      expect(
        blackPos.isLegal(const NormalMove(from: Square.e8, to: Square.g8)),
        true,
      );
      expect(
        blackPos.isLegal(const NormalMove(from: Square.e8, to: Square.c8)),
        true,
      );
      expect(
        blackPos.isLegal(const NormalMove(from: Square.e8, to: Square.h8)),
        true,
      );
      expect(
        blackPos.isLegal(const NormalMove(from: Square.e8, to: Square.a8)),
        true,
      );
    });
    ...
    group('play', () {
      test('a move not valid', () {
        expect(
          () => Chess.initial.play(
            const NormalMove(from: Square.e4, to: Square.e6),
          ),
      ...
      test('fullmoves increment', () {
        ...
        expect(
          pos.play(const NormalMove(from: Square.e7, to: Square.e5)).fullmoves,
          2,
        );
      });
      ...
      test('castling move', () {
        ...
        expect(
          pos.castles.castlingRights.isIntersected(
      ...
      test('castling moves', () {
        final pos = Chess.fromSetup(
          Setup.parseFen('2r5/8/8/8/8/8/6PP/k2KR3 w K -'),
        );
  ...
  group('Racing Kings', () {
    ...
    test('game ongoing', () {
      ...
      final position = RacingKings.fromSetup(
        Setup.parseFen('1K6/7k/8/8/8/8/8/8 b - - 0 1'),
      );
  ...
  group('Three check', () {
    ...
    test('remaining checks', () {
      ...
      expect(
        pos.play(NormalMove.fromUci('f1b5')).fen,
