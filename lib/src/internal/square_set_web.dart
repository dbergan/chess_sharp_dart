...
/// Web specific implementation of [SquareSet] using [BigInt].
extension type const SquareSet(BigInt value) {
  const SquareSet.fromBigInt(BigInt b) : value = b;

  /// Creates a [SquareSet] with a single [Square].
  SquareSet.fromSquare(Square square) : value = BigInt.one << square.value;

  /// Creates a [SquareSet] from several [Square]s.
  SquareSet.fromSquares(Iterable<Square> squares)
    : value = squares
          .map((square) => BigInt.one << square.value)
  ...
  SquareSet.fromRank(Rank rank)
    : value = BigInt.parse('ff', radix: 16) << (8 * rank.value);

  /// Create a [SquareSet] containing all squares of the given file.
  SquareSet.fromFile(File file)
    : value = BigInt.parse('0101010101010101', radix: 16) << file.value;
  ...
  static final lightSquares = SquareSet(
    BigInt.parse('55AA55AA55AA55AA', radix: 16),
  );
  static final darkSquares = SquareSet(
    BigInt.parse('AA55AA55AA55AA55', radix: 16),
  );
  static final diagonal = SquareSet(
    BigInt.parse('8040201008040201', radix: 16),
  );
  static final antidiagonal = SquareSet(
    BigInt.parse('0102040810204080', radix: 16),
  );
  ...
  static final backranks = SquareSet(
    BigInt.parse('ff000000000000ff', radix: 16),
  );
  static final firstRank = SquareSet(BigInt.parse('ff', radix: 16));
  static final eighthRank = SquareSet(
    BigInt.parse('ff00000000000000', radix: 16),
  );
  ...
  static final ranksThreeToEight = SquareSet(
    BigInt.parse('ffffffffff0000', radix: 16),
  );
  static final ranksOneToSix = SquareSet(
  ...
  bool has(Square square) {
    return (value & (BigInt.one << square.value)) != BigInt.zero;
  }
  ...
  SquareSet withSquare(Square square) {
    return SquareSet(value | (BigInt.one << square.value));
  }
  ...
  Iterable<Square> _iterateSquares() sync* {
    ...
    while (bitboard != BigInt.zero) {
      ...
      bitboard ^= BigInt.one << square!.value;
      yield square;
    }
  }
